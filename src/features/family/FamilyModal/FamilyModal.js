...
export default class FamilyModal extends Component {
  ...
  render() {
    ...
    const renderMaidenName =
      person.maidenName !== undefined ? (
        <span>
          <b>Maiden Name:</b> {person.maidenName}
        </span>
      ) : null;
    const renderParents =
      person.parents &&
      person.parents.length &&
      person.parents[0].parentName ? (
        <span>
          {person.firstName} is the{' '}
          {person.gender === 'male' ? 'son' : 'daughter'} of{' '}
          {person.parents[0].parentName}
          {person.parents &&
          person.parents.length > 1 &&
          person.parents[1].parentName
    ...
    const renderSiblings =
      person.siblings && person.siblings.length ? (
        <span>
          <span>
            {person.gender === 'male' ? 'He' : 'She'} has{' '}
            {person.siblings.length}{' '}
            {person.siblings.length === 1 ? 'sibling' : 'siblings'}:{' '}
          </span>
    ...
    const renderChildren =
      person.children && person.children.length ? (
        <span>
          ...
          person.spouses[0].spouseName ? (
            <span>
              {' '}
              {person.spouses[0].type} {person.spouses[0].spouseName}
              <span>
                {' '}
                and had {person.children.length}{' '}
                {person.children.length === 1 ? 'child' : 'children'}:{' '}
              </span>
              {person.children.map((item, index) =>
                item.childName !== undefined ? (
                  <span key={index}>
                    {(index ? ', ' : '') + item.childName}
                  </span>
                ) : null
              )}
              .
    ...
    const renderSuffix =
      person.suffix !== undefined ? <span>{person.suffix}</span> : null;
